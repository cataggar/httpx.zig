...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    // protocol_version
    mem.writeInt(u16, client_hello_buf[ch_off..][0..2], @intFromEnum(tls.ProtocolVersion.tls_1_2), .big);
    ch_off += 2;
    // client_random
    @memcpy(client_hello_buf[ch_off..][0..32], &client_hello_rand);
    ch_off += 32;
    // legacy_session_id
    client_hello_buf[ch_off] = 32;
    ch_off += 1;
    @memcpy(client_hello_buf[ch_off..][0..32], &legacy_session_id);
    ch_off += 32;
    // cipher_suites
    @memcpy(client_hello_buf[ch_off..][0..cipher_suites.len], &cipher_suites);
    ch_off += cipher_suites.len;
    // compression methods
    client_hello_buf[ch_off] = 1;
    ch_off += 1;
    client_hello_buf[ch_off] = 0; // null compression
    ch_off += 1;

    // extensions
    const ext_payload_len = extensions_payload.len;
    const sni_ext_len: usize = switch (options.host) {
        .no_verification => 0,
    ...
    ch_off += 2;
    // extensions payload
    @memcpy(client_hello_buf[ch_off..][0..ext_payload_len], &extensions_payload);
    ch_off += ext_payload_len;
    // server name extension (skip if no_verification)
    if (sni_ext_len > 0) {
        @memcpy(client_hello_buf[ch_off..][0..sni_ext_len], &server_name_extension);
        ch_off += sni_ext_len;
    }
    // ALPN extension
    if (alpn_extension.len > 0) {
        @memcpy(client_hello_buf[ch_off..][0..alpn_extension.len], alpn_extension);
            ...
            .handshake => {
                ...
                switch (handshake_cipher) {
                    inline else => |*p| {
                        ...
                        P.AEAD.decrypt(cleartext, ciphertext, auth_tag, record_header, nonce, pv.server_handshake_key) catch return error.TlsBadRecordMac;
                    ...
                    .server_hello => {
                        ...
                        switch (cipher_suite_tag) {
                            ...
                            => |tag| {
                                ...
                                p.transcript_hash.update(cleartext_header[tls.record_header_len..]); // Client Hello
        ...
        inline else => |*p| switch (c.tls_version) {
            .tls_1_3 => {
                ...
                while (true) {
                    ...
                    c.write_seq += 1;
                    // TLS 1.3 key update: at 2^64 records the implementation MUST
                    // send a KeyUpdate. This is practically unreachable (584K years
                    // at 1 record/ms), but we guard with a debug assertion.
                    if (std.debug.runtime_safety) {
                        if (c.write_seq == 0) @panic("TLS 1.3 sequence number overflow: key update required");
                    }
                }
            },
            .tls_1_2 => {
