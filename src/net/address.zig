...
pub fn resolve(hostname: []const u8, port: u16) !Address {
    if (parseIp4(hostname)) |ip4| {
        return Address.initIp4(ip4, port);
    }

    if (parseIp6(hostname)) |ip6| {
        return Address.initIp6(ip6, port);
    }

    // DNS resolution is not available without std.net; return an error.
...
pub fn formatAddress(addr: Address, allocator: Allocator) ![]u8 {
    _ = addr;
