...
pub fn init() NetInitError!void {
    ...
    compat_mod.wsaInit() catch return error.InitializationError;
}
...
pub const Socket = struct {
    ...
    pub fn createForAddress(addr: Address) !Self {
        try init();
        const handle = try compat.socketCreate(addr.any.family, posix.SOCK.STREAM, 0);
        return .{ .handle = handle };
    }
    ...
    pub fn send(self: *Self, data: []const u8) !usize {
        return compat.socketSend(self.handle, data);
    }
    ...
    pub fn recv(self: *Self, buffer: []u8) !usize {
        return compat.socketRecv(self.handle, buffer);
    }

    /// Sets a socket option.
    pub fn setOption(self: *Self, level: u32, optname: u32, value: []const u8) !void {
        try compat.socketSetSockOpt(self.handle, @intCast(level), optname, value);
    }
    ...
    pub fn setNoDelay(self: *Self, enable: bool) !void {
        ...
        try compat.socketSetSockOpt(self.handle, @intCast(posix.IPPROTO.TCP), posix.TCP.NODELAY, std.mem.asBytes(&value));
    }
    ...
    pub fn setRecvTimeout(self: *Self, ms: u64) !void {
        if (is_windows) {
            ...
            try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.RCVTIMEO, std.mem.asBytes(&value_ms));
        } else {
            const tv = posix.timeval{
                .sec = @intCast(ms / 1000),
                .usec = @intCast((ms % 1000) * 1000),
            };
            try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.RCVTIMEO, std.mem.asBytes(&tv));
        }
    }
    ...
    pub fn setSendTimeout(self: *Self, ms: u64) !void {
        if (is_windows) {
            const value_ms: u32 = @intCast(@min(ms, @as(u64, std.math.maxInt(u32))));
            try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.SNDTIMEO, std.mem.asBytes(&value_ms));
        } else {
            const tv = posix.timeval{
                .sec = @intCast(ms / 1000),
                .usec = @intCast((ms % 1000) * 1000),
            };
            try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.SNDTIMEO, std.mem.asBytes(&tv));
        }
    }
    ...
    pub fn setKeepAlive(self: *Self, enable: bool) !void {
        const value: u32 = if (enable) 1 else 0;
        try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.KEEPALIVE, std.mem.asBytes(&value));
    }

    /// Enables or disables address reuse.
    pub fn setReuseAddr(self: *Self, enable: bool) !void {
        const value: u32 = if (enable) 1 else 0;
        try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), @intCast(posix.SO.REUSEADDR), std.mem.asBytes(&value));
    }
...
pub const TcpListener = struct {
    ...
    pub fn getLocalAddress(self: *Self) !Address {
        ...
        return Address{ .any = addr, .len = addr_len };
    }
};
...
pub const UdpSocket = struct {
    ...
    pub fn sendTo(self: *Self, addr: Address, data: []const u8) !usize {
        return compat.socketSendTo(self.handle, data, &addr.any, addr.getOsSockLen());
    }
    ...
    pub fn recvFrom(self: *Self, buffer: []u8) !struct { n: usize, addr: Address } {
        var addr: posix.sockaddr = undefined;
        var addr_len: posix.socklen_t = @sizeOf(posix.sockaddr);
        const n = try compat.socketRecvFrom(self.handle, buffer, &addr, &addr_len);
        return .{ .n = n, .addr = Address{ .any = addr, .len = addr_len } };
    }

    /// Enables or disables address reuse.
    pub fn setReuseAddr(self: *Self, enable: bool) !void {
        const value: u32 = if (enable) 1 else 0;
        try compat.socketSetSockOpt(self.handle, @intCast(posix.SOL.SOCKET), posix.SO.REUSEADDR, std.mem.asBytes(&value));
    }
...
test "TcpListener getLocalAddress" {
    var listener = try TcpListener.init(try Address.parseIp("127.0.0.1", 0));
...
test "UdpSocket send/recv localhost" {
    ...
    try recv_sock.bind(try Address.parseIp("127.0.0.1", 0));
