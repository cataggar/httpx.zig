...
- **Protocol Support**: High-level HTTP/1.0/1.1/2 client/server paths plus experimental HTTP/3/QPACK/QUIC codec primitives.
...
Zig's standard library does not provide HTTP/2, HTTP/3, QUIC, or TLS/ALPN support. httpx.zig implements HTTP/2 and TCP TLS plus selected unauthenticated HTTP/3/QPACK/QUIC codec primitives:
...
- **HTTP/3** frame, SETTINGS, GOAWAY, and CONNECTION_CLOSE codecs; public H3 runtimes are rejected
:::
